 ...
 */
        ...
        public:
            ...
            explicit Stream(Context& context, Args&& ... args)
                    : m_context(context), m_nextLayer(std::forward<Args>(args)...), m_core(m_receive_buffer, m_send_buffer, m_context), m_input_buffer_space(MAX_CIPHERTEXT_SIZE, '\0'), m_input_buffer(m_input_buffer_space.data(), m_input_buffer_space.size()) {
            }
            ...
            explicit Stream(Arg&& arg, Context& context)
                    : m_context(context), m_nextLayer(std::forward<Arg>(arg)), m_core(m_receive_buffer, m_send_buffer, m_context), m_input_buffer_space(MAX_CIPHERTEXT_SIZE, '\0'), m_input_buffer(m_input_buffer_space.data(), m_input_buffer_space.size()) {
            }
            ...
            void handshake(Connection_Side side, boost::system::error_code& ec) {
                ...
                while (!native_handle()->is_active() && !ec) {
                    ...
                    send_pending_encrypted_data(errc);
        ...
        protected:
            ...
            public:
                ...
                bool tls_session_established(const Botan::TLS::Session& e) override {
                    if(m_tls_context.m_tls_established_callback){
                        m_tls_context.m_tls_established_callback(e);
                    }
                    return true;
                }
